...
template <typename reset_pid, typename move_and_measure, double k, double k_prim>
double weight(const PID& pid)
{
    if(pid.p < 0)
    {
...
int main(int argc, char* argv[])
{
    ...
    auto res = simulated_annealing<Space_element, double, double>(n_iterations, initial_pid,
                                        std::bind(weight<reset_pid_distance, move_and_measure_distance, 0.005, 0.01>, std::placeholders::_1),
